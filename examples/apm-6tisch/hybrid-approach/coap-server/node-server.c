...
PROCESS(er_example_server, "Server | APM-6TiSCH Hybrid Approach");
...
PROCESS_THREAD(er_example_server, ev, data)
{
  ...
  LOG_INFO("Starting Coap-6TiSCH Server - Hybrid Approach\n");

  coap_activate_resource(&res_hello, "test/hello");
  coap_activate_resource(&res_send_dummy, "send/dummy");
  coap_activate_resource(&res_send_telemetry, "send/telemetry");
